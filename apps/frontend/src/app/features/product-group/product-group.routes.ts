...
export const productGroupRoutes: Route[] = [
  {
    path: '',
    loadComponent: () =>
      import('./overview/product-group-overview.component').then(
        (m) => m.ProductGroupOverviewComponent
      ),
  ...
  },
  {
    path: ':id',
    loadComponent: () =>
      import('./details/product-group-details.component').then(
        (m) => m.ProductGroupDetailsComponent
      ),
    canActivate: [AuthenticationGuard, RoleGuard],
    data: { roles: [AuthRoles.SUSTAINABILITY_MANAGER, AuthRoles.BUYER] },
  },
